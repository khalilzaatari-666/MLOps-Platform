...
async def create_dataset(
    dataset_request: schemas.CreateDatasetRequest,  # Change to receive as a body model
...
@app.get("/list_images/{dataset_id}", response_model=List[schemas.ImageResponse])
async def list_images(dataset_id: int, db: Session = Depends(get_db)):
    images = crud.list_images(dataset_id=dataset_id)  # List images for the provided dataset ID
    return images

@app.get("/list_datasets/", response_model=List[schemas.DatasetResponse])
